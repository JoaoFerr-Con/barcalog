...
function TelaLogin({ aoEntrar }) {
  ...
  function enviar(e) {
    ...
    if (!t) { setErro("Transportadora ou senha inválida."); return; }
    aoEntrar(t);
  }

  return (
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "var(--fundo)" }}>
      <form onSubmit={enviar} className="cartao" style={{ width: 360 }}>
        <div className="cartao__cabecalho">
          <h3>Portal do Transportador</h3>
          <p>BarcaLog — Porto de Barcarena</p>
        </div>
        <div className="cartao__corpo" style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          <label style={{ fontSize: 12, fontWeight: 600 }}>
            Nome da transportadora
            <input value={nome} onChange={e => setNome(e.target.value)} style={estiloInput} placeholder="Ex: Rota Amazônia Cargas" />
          </label>
          <label style={{ fontSize: 12, fontWeight: 600 }}>
            Senha
...
export default function PortalTransportadora() {
  ...
  if (!logada) return <TelaLogin aoEntrar={setLogada} />;
