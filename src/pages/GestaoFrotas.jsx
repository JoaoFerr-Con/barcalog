...
export default function GestaoFrotas() {
  ...
  const [condutorForm, setCondutorForm] = useState({ nome: "", transportadora: "", placaVinculada: "" });
  ...
  function salvarCondutor(e) {
    e.preventDefault();
    if (!condutorForm.nome || !condutorForm.transportadora) return;
    cadastrarCondutor(condutorForm);
    notificar(`Condutor ${condutorForm.nome} cadastrado.`, "sucesso");
    setCondutorForm({ nome: "", transportadora: "", placaVinculada: "" });
  }

  return (
    <>
      <AvisoDadosSimulados>
        Os veículos e condutores cadastrados aqui são um cadastro de exemplo, usado pelo Sistema de Negativação e pela Portaria.
      </AvisoDadosSimulados>
      ...
      <div className="pilha">
        <form onSubmit={salvarCondutor} className="cartao">
          <div className="cartao__cabecalho">
            ...
            <p>Motoristas monitorados pelo Sistema de Negativação</p>
          </div>
          <div className="cartao__corpo" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <label style={{ fontSize: 12, fontWeight: 600, gridColumn: "1 / -1" }}>
              Nome
        ...
        <div className="cartao">
          ...
          <div className="cartao__corpo" style={{ overflowX: "auto" }}>
            <table>
              <thead><tr><th>Nome</th><th>Transportadora</th><th>Placa</th><th></th></tr></thead>
              <tbody>
                {condutores.map(c => (
                  <tr key={c.id}>
                    <td>{c.nome}</td>
