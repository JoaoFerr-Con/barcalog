...
import {
  ...
  custoTotalDemurrage, CAPACIDADE_DIARIA, picosEntradaSaida, indicadoresPerformance
...
export default function VisaoGeral() {
  ...
  const picos = useMemo(() => picosEntradaSaida(registros), [registros]);
      ...
      {modal === "mesMov" && (
        <Modal titulo={`Mês Mais Movimentado: ${kpis.mesMaisMovimentado.rotulo}`} subtitulo="Detalhamento do mês de pico" aoFechar={() => setModal(null)}>
          <p style={{ fontSize: 13, color: "var(--tinta-suave)", marginBottom: 14 }}>
            ...
            da média mensal do período. Custo estimado de retenção no período todo: <b>{formatarBRL(demurrage.custoTotal)}</b> ({demurrage.carretasRetidas} carretas retidas além de 144h).
          </p>
          <GraficoLinhas
            altura={140}
      ...
      {aba === "analises" && (
        <div className="grade-painel conteudo-pagina">
          <div className="pilha">
            <div className="cartao">
              <div className="cartao__cabecalho">
                ...
                <p>Projeção com base na tendência dos últimos meses (regressão linear) — {preditiva.tendencia === "alta" ? "tendência de alta" : preditiva.tendencia === "queda" ? "tendência de queda" : "tendência estável"}. Com menos de um ano de histórico, ainda não dá pra detectar sazonalidade de calendário real.</p>
              </div>
              <div className="cartao__corpo">
                <GraficoLinhas
                  altura={160}
                  rotulosX={todosComProjecao.map(m => m.rotulo)}
                  series={[
                    {
                      nome: "Histórico",
                      cor: "var(--navio-700)",
                      valores: [...porMes.map(m => m.total), ...preditiva.projecoes.map(() => null)]
                    },
                    {
                      nome: `Projeção (${preditiva.tendencia === "alta" ? "tendência de alta" : preditiva.tendencia === "queda" ? "tendência de queda" : "estável"})`,
                      cor: "var(--ambar-500)",
                      tracejado: true,
                      valores: [...porMes.map((_, i) => i === porMes.length - 1 ? porMes[i].total : null), ...preditiva.projecoes.map(p => p.total)]
                    }
                  ]}
                />
                <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginTop: 10 }}>
                  {preditiva.projecoes.map(p => (
                    <span key={p.chave} style={{ fontSize: 11, padding: "3px 8px", borderRadius: 8, background: "var(--superficie-alt)", color: "var(--tinta-suave)" }}>
                      {p.rotulo}: <b style={{ color: "var(--tinta)" }}>{p.total.toLocaleString("pt-BR")}</b> ({p.confianca}% confiança)
                    </span>
                  ))}
                </div>
              </div>
          ...
          <div className="pilha">
            ...
            <div className="cartao">
              <div className="cartao__cabecalho"><h3>Horários de Pico — Entrada x Saída</h3><p>Marcações (entrada) e liberações (saída) por hora do dia</p></div>
              <div className="cartao__corpo">
                ...
                <GraficoLinhas
                  altura={170}
                  rotulosX={picos.entrada.map(h => `${h.hora}h`)}
                  series={[
                    { nome: "Entrada (marcação)", cor: "var(--navio-700)", valores: picos.entrada.map(h => h.total) },
                    { nome: "Saída (liberação)", cor: "var(--verde-500)", valores: picos.saida.map(h => h.total) }
                  ]}
                />
              </div>
            </div>
