            ...
            <div className="cartao">
              ...
              <div className="cartao__corpo">
                ...
                <p style={{ fontSize: 10.5, color: "var(--tinta-fraca)", marginTop: 4 }}>
                  Faixa sombreada = intervalo de confiança (~80%), crescendo quanto mais distante a projeção.
                </p>
                <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginTop: 10 }}>
                  {preditiva.projecoes.map(p => (
                    <span key={p.chave} style={{ fontSize: 11, padding: "3px 8px", borderRadius: 8, background: "var(--superficie-alt)", color: "var(--tinta-suave)" }}>
                      {p.rotulo}: <b style={{ color: "var(--tinta)" }}>{p.total.toLocaleString("pt-BR")}</b>
                      <span style={{ color: "var(--tinta-fraca)" }}> ({p.margemInferior.toLocaleString("pt-BR")}–{p.margemSuperior.toLocaleString("pt-BR")}, {p.confianca}% conf.)</span>
                    </span>
                  ))}
                </div>
              </div>
