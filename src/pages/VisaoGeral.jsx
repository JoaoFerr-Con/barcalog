      ...
      {aba === "resumo" && (
        <div className="grade-painel conteudo-pagina">
          ...
          <div className="pilha">
            ...
            <div className="cartao">
              <div className="cartao__cabecalho"><h3>Indicadores de Performance</h3><p>TMA, Δ vs mês anterior, Taxa D0 (≤24h) e volume, por terminal</p></div>
              <div className="cartao__corpo" style={{ overflowX: "auto" }}>
                  ...
                  <tbody>
                    {desempenho.map(d => (
                      <tr key={d.terminal}>
                        ...
                        <td>{d.totalGeral.toLocaleString("pt-BR")}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                <p style={{ fontSize: 11, color: "var(--tinta-fraca)", marginTop: 10 }}>
                  Δ TMA vermelho = tempo de espera piorou vs mês anterior. Δ D0 verde = mais veículos liberados dentro de 24h (pp = pontos percentuais).
                </p>
              </div>
            </div>
            <div className="cartao">
          ...
          <div className="pilha">
            ...
            <div className="cartao">
              ...
              <div className="cartao__corpo">
                ...
                <p style={{ fontSize: 11.5, color: "var(--tinta-suave)", marginBottom: 4 }}>Entrada (marcação)</p>
                <div className="grafico-fluxo" style={{ gap: 2, height: 90 }}>
                  {picos.entrada.map(h => (
                    <div key={h.hora} className="grafico-fluxo__col">
                      <div className="grafico-fluxo__barra" style={{ height: `${(h.total / picos.picoEntrada.total) * 100}%`, background: h.hora === picos.picoEntrada.hora ? "var(--navio-700)" : h.acimaMedia ? "var(--azul-500)" : undefined }} />
                      <span className="grafico-fluxo__rotulo" style={{ fontSize: 9 }}>{h.hora}</span>
                    </div>
                  ))}
                </div>
                <p style={{ fontSize: 11.5, color: "var(--tinta-suave)", margin: "14px 0 4px" }}>Saída (liberação)</p>
                <div className="grafico-fluxo" style={{ gap: 2, height: 90 }}>
                  {picos.saida.map(h => (
                    <div key={h.hora} className="grafico-fluxo__col">
                      <div className="grafico-fluxo__barra" style={{ height: `${(h.total / picos.picoSaida.total) * 100}%`, background: h.hora === picos.picoSaida.hora ? "var(--verde-500)" : h.acimaMedia ? "#7ED6A5" : undefined }} />
                      <span className="grafico-fluxo__rotulo" style={{ fontSize: 9 }}>{h.hora}</span>
                    </div>
                  ))}
                </div>
