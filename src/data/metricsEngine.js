...
export function analisePreditiva(registros, mesesAFrente = 3) {
  ...
  let num = 0, den = 0;
  for (let i = 0; i < n; i++) { num += (i - mediaX) * (valores[i] - mediaY); den += (i - mediaX) ** 2; }
  const inclinacao = den === 0 ? 0 : num / den;
  const intercepto = mediaY - inclinacao * mediaX;
  const fatoresSazonais = valores.map((v, i) => { const e = intercepto + inclinacao * i; return e > 0 ? v / e : 1; });
  ...
  for (let p = 1; p <= mesesAFrente; p++) {
    ...
    const base = intercepto + inclinacao * (n - 1 + p);
    const fator = fatoresSazonais[Math.min((n - 1 + p) % n, fatoresSazonais.length - 1)];
    projecoes.push({
      chave: `${ano}-${String(mes).padStart(2, "0")}`,
      rotulo: `${NOMES[mes - 1]}/${String(ano).slice(2)}`,
      total: Math.max(0, Math.round(base * fator)),
      projetado: true,
      confianca: Math.max(60, 95 - p * 8)
    });
  }
  return { projecoes, tendencia: inclinacao > 50 ? "alta" : inclinacao < -50 ? "queda" : "estavel", inclinacao: Math.round(inclinacao) };
}
