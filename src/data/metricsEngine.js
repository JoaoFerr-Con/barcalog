...
// ---------- E. HEATMAP: TURNOS × DIA DA SEMANA ----------
// Conta retenções (>24h) por cruzamento hora-do-dia × dia-da-semana.
export function heatmapTurnos(registros) {
  // Agrupa em turnos de 4h pra caber no relatório
  const turnos = [
    { rotulo: "00–04h", min: 0, max: 4 },
    { rotulo: "04–08h", min: 4, max: 8 },
    { rotulo: "08–12h", min: 8, max: 12 },
    { rotulo: "12–16h", min: 12, max: 16 },
    { rotulo: "16–20h", min: 16, max: 20 },
    { rotulo: "20–00h", min: 20, max: 24 }
  ];
  const grade = turnos.map(t => ({
    ...t,
    dias: DIAS_SEMANA.map(() => 0)
  }));
  registros.forEach(m => {
    if (m.esperaHoras < 24) return; // só conta retenções
    const dt = new Date(m.marcadoEm);
    const hora = dt.getHours();
    const diaSemana = dt.getDay();
    const turnoIdx = turnos.findIndex(t => hora >= t.min && hora < t.max);
    if (turnoIdx >= 0) grade[turnoIdx].dias[diaSemana] += 1;
  });
  const maiorValor = Math.max(1, ...grade.flatMap(t => t.dias));
  return { grade, diasRotulos: DIAS_SEMANA, maiorValor };
}
