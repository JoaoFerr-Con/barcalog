...
export function indiceRiscoGargalo(registros) {
  const ritmo = ritmoOperacional(registros);
  return ritmo.map(r => {
    // ρ = λ/μ é a métrica real de teoria das filas: chegada sobre
    // liberação. Índice é só ρ×100, direto — nada de peso arbitrário
    // combinando outra coisa junto. Mais simples de auditar.
    const rho = r.mu > 0 ? r.lambda / r.mu : (r.lambda > 0 ? 2 : 0);
    const indice = Math.round(rho * 100);
    let nivel = "normal";
    if (rho >= 1.4) nivel = "critico";
    else if (rho >= 1.2) nivel = "alto";
    else if (rho >= 1.0) nivel = "atencao";
    return {
      hora: r.hora,
      lambda: r.lambda,
      mu: r.mu,
