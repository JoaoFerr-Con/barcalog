...
// Dados de demonstração dependentes de tempo (statusPortariaDesde) ficam
// "congelados" na primeira vez que o navegador roda o seed e salva em
// localStorage — depois disso, o tempo real segue passando e o "há quanto
// tempo" vira um número absurdo (dias/semanas), já que ninguém no mundo
// real atualiza a placa demo. Pra não vazar esse artefato de simulação na
// tela, normalizamos: qualquer statusPortariaDesde com mais de 12h de
// "idade" é re-simulado com um valor recente plausível (respeitando a
// ordem relativa original), toda vez que os dados são carregados.
function normalizarDadosDemo(dados) {
  const LIMITE_MS = 12 * 3600000;
  const agora = Date.now();
  const veiculosOrdenados = [...dados.veiculos].sort(
    (a, b) => new Date(a.statusPortariaDesde) - new Date(b.statusPortariaDesde)
  );
  const algumVencido = veiculosOrdenados.some(v => agora - new Date(v.statusPortariaDesde).getTime() > LIMITE_MS);
  if (!algumVencido) return dados;

  // Redistribui offsets plausíveis (20min a 6h atrás), mantendo a mesma
  // ordem de chegada que o seed original tinha.
  const offsetsMinutos = [20, 90, 120, 180, 240, 300, 360];
  veiculosOrdenados.forEach((v, i) => {
    ...
    if (emAndamento) {
      const offset = offsetsMinutos[i % offsetsMinutos.length];
      v.statusPortariaDesde = new Date(agora - offset * 60000).toISOString();
    } else {
      v.statusPortariaDesde = new Date(agora - 6 * 3600000).toISOString();
    }
  });
  return dados;
}
...
function carregar() {
  try {
    ...
    return normalizarDadosDemo(JSON.parse(bruto));
  } catch {
