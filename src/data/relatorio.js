...
export function totaisPorOperador(registros) {
  const mapa = {};
  registros.forEach(m => {
    // Bunge e Amaggi consolidados numa única categoria — a granularidade
    // separada não agregava valor na visão geral e só poluía o gráfico.
    const chave = (m.operador === "BUNGE" || m.operador === "AMAGGI") ? "BUNGE / AMAGGI" : m.operador;
    mapa[chave] = (mapa[chave] || 0) + 1;
  });
  return Object.entries(mapa)
...
// Detalhamento diário por mês — gera um array de marcações por dia pra
// cada mês do período. Cada entrada tem { dia: 1..31, total }. É o que
// alimenta os gráficos de barra diários no relatório de impressão, igual
// ao PDF de referência (seção "Detalhamento Diário por Mês").
export function detalhamentoDiarioPorMes(registros) {
  const porMesChave = {};
  registros.forEach(m => {
    const chave = chaveMes(m.marcadoEm);
    const dia = new Date(m.marcadoEm).getDate();
    if (!porMesChave[chave]) porMesChave[chave] = {};
    porMesChave[chave][dia] = (porMesChave[chave][dia] || 0) + 1;
  });

  const chavesOrdenadas = Object.keys(porMesChave).sort();
  return chavesOrdenadas.map(chave => {
    ...
    for (let d = 1; d <= diasNoMes; d++) {
      const qtd = porMesChave[chave][d] || 0;
      dias.push({ dia: d, total: qtd });
      total += qtd;
    }
    return {
      chave,
      rotulo: `${NOMES_MES[mesNum - 1]} ${ano}`,
      total,
      dias
    };
  });
